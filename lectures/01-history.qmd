...
![The scope of this course: the intersection of Computer Vision and Deep Learning, within the broader fields of Machine Learning and Artificial Intelligence. *Drawn for these notes, after the CS231n 2025 Lecture 1 slides.*](../figures/01-history/diagram_01.svg){#fig-course-scope width="90%"}
...
![Larry Roberts' PhD thesis (1963): extracting 3D structure from 2D images of polyhedral "blocks world" scenes. *Source: scan of Pictures 3A--3D from L. G. Roberts, "Machine Perception of Three-Dimensional Solids", PhD thesis, MIT, 1963.*](../figures/01-history/roberts_1963_blocks.png){#fig-roberts-blocks width="60%"}
...
![The Mark I Perceptron (1958). Left: Frank Rosenblatt adjusting the machine's wiring. Right: Architecture diagram showing sensory units (S-Units) receiving input from the retina, association units (A-Units) with random connections, and response units (R-Units) producing output. *Source: composite of an archival Cornell University photograph and Figure 1 of the Mark I Perceptron report, Cornell Aeronautical Laboratory.*](../figures/01-history/rosenblatt_perceptron_1958.png){#fig-perceptron-1958 width="85%"}
...
![The Minsky-Papert critique (1969). Left: Mathematical formalization of the perceptron---inputs $\varphi_1, \ldots, \varphi_n$ are weighted, summed, and thresholded to produce output $\psi$. Right: The influential book "Perceptrons: An Introduction to Computational Geometry" that demonstrated fundamental limitations of single-layer networks. *Source: figures from and the cover of M. Minsky & S. Papert, "Perceptrons", MIT Press, 1969.*](../figures/01-history/minsky_papert_1969.jpg){#fig-minsky-papert width="80%"}
...
![(a) Fukushima's Neocognitron (1980): alternating layers of S-cells and C-cells, directly inspired by Hubel & Wiesel's hierarchy. (b) LeCun's LeNet (1989): similar architecture but with learnable parameters via backpropagation. The structural similarity is striking---the key difference is how parameters were set. *Source: (a) after Fukushima (1980); (b) a colourised redraw of Figure 2 of LeCun et al. (1998).*](../figures/01-history/neocognitron_lenet.png){#fig-neocognitron-lenet width="90%"}
...
![A network trained with backpropagation to solve XOR---the very problem Minsky and Papert proved impossible for single-layer perceptrons. The numbers show weights learned automatically: 2 input units, 2 hidden units, 1 output unit. *Source: Rumelhart, Hinton & Williams, "Learning representations by back-propagating errors", Nature 323, 1986.*](../figures/01-history/backprop_1986.png){#fig-backprop width="45%"}
...
![LeNet-5 architecture (LeCun et al., 1998). A 32×32 input image passes through convolutional layers (C1, C3) that extract features, subsampling layers (S2, S4) that reduce spatial dimensions, and fully connected layers (C5, F6) that produce the final classification into 10 digit classes. *Source: colourised redraw of Figure 2 of LeCun, Bottou, Bengio & Haffner, "Gradient-Based Learning Applied to Document Recognition", Proc. IEEE 86(11), 1998.*](../figures/01-history/lenet_1998.png){#fig-lenet width="95%"}
...
![ImageNet: a mosaic of sample images forming the dataset's name. The sheer diversity and scale of ImageNet---15 million images across 22,000 categories---was unprecedented and proved essential for training deep networks. *Source: promotional mosaic from image-net.org; dataset from Deng et al., CVPR 2009.*](../figures/01-history/imagenet_mosaic.jpg){#fig-imagenet width="85%"}
...
![AlexNet architecture (Krizhevsky, Sutskever & Hinton, 2012). Input 224×224×3 images pass through 5 convolutional layers and 3 fully connected layers, outputting 1000 class probabilities. The network was split across two GPUs (top/bottom paths) due to memory constraints---a detail that influenced the design. *Source: Figure 2 of Krizhevsky, Sutskever & Hinton, "ImageNet Classification with Deep Convolutional Neural Networks", NeurIPS 2012.*](../figures/01-history/alexnet_2012.png){#fig-alexnet width="95%"}
